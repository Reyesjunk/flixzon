...
export const fetchShows = query => dispatch => {
  ...
  return fetch(`http://api-public.guidebox.com/v2/search?type=show&query=${query}&api_key=${API_KEY}`, {
    mode: 'cors'
  })
  .then(res => res.json())
  .then((res) => {
...
function fetchProvider (id, provider) {
  ...
  return fetch(`http://api-public.guidebox.com/v2/shows/${id}/episodes?sources=${provider}&api_key=${API_KEY}`, {
    mode: 'cors'
  })
  .then(res => res.json())
  .then(res => {
